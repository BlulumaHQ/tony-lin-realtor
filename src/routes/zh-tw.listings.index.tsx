...
function ListingGroup({ title, items }: { title: string; items: typeof listings }) { if (!items.length) return null; return <section className="mx-auto max-w-7xl px-6 pb-16"><h2 className="font-serif text-2xl">{title}</h2><div className="mt-6 grid gap-8 md:grid-cols-2">{items.map((item) => <ListingCard key={item.id} listing={item} locale="zh-TW" />)}</div></section>; }
...
export function LeadForm({ compact = false, source }: LeadFormProps) {
  ...
  return (
    <form
      ...
      className={compact ? "" : "rounded-3xl border border-border bg-card p-7 shadow-sm md:p-10"}
    >
      ...
      <Button
        ...
        className={compact ? "mt-5 w-full rounded-full md:w-auto" : "mt-7 w-full rounded-full"}
      >
        {status === "sending"
          ? "Sending…"
          : source === "valuation"
